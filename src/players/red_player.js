  ...
  ai: function(playerState, enemiesStates, gameEnvironment) {
    ...
    enemiesStates.forEach(function(enemy) {
      if (enemy.isAlive === true) {
        enemy.nearestAmmoDistance = calculateDistanceToNearestAmmo(
          enemy.position
        )
        enemies.push(enemy)
      }
    ...
    // Process parameters
    params.vulnerabilityLevel = calculateVulnerabilityLevel(
      playerState.position
    )
    params.canKill =
      playerState.ammo > 0 && utils.canKill(playerState, enemiesStates)
    ...
    function calculateVulnerabilityLevel(targetPosition) {
      ...
      enemies.forEach(function(enemy) {
        if (
          utils.isVisible(enemy.position, targetPosition, enemy.direction) &&
          enemy.ammo > 0
        ) {
          vulnerabilityLevel = Math.max(vulnerabilityLevel, 1.0)
        } else if (
          isAligned(enemy.position, targetPosition) &&
          (enemy.ammo > 0 || enemy.nearestAmmoDistance === 1)
        ) {
      ...
      if (vulnerabilityLevel === 0.0) {
        ...
        enemies.forEach(function(enemy) {
          if (enemy.ammo > 0) {
            if (
              isAligned(enemy.position, northEast) ||
              isAligned(enemy.position, southWest)
            ) {
    ...
    function defend() {
      if (params.vulnerabilityLevel === 1.0) {
        ...
        if (
          attacker &&
          ORIENTATION[attacker.direction] === ORIENTATION[playerState.direction]
        ) {
          canRun = false
        }
    ...
    function getSafestDirection() {
      ...
      options.forEach(function(option) {
        ...
        var isBetterOption =
          option.vulnerability < lowestVulnerability ||
          (option.vulnerability === lowestVulnerability &&
            option.distanceLeft > maxDistanceLeft)
    ...
    function moveWisely() {
      if (playerState.ammo === 0 && params.nearestAmmo) {
        ...
        if (
          params.nearestAmmo &&
          params.nearestAmmo.distance < params.nearestEnemy.distance
        ) {
          return getAmmo(params.nearestAmmo.position)
        } else {
    ...
    function huntEnemy(enemy) {
      if (
        ...
        if (
          attackDirection === playerState.direction &&
          (enemyDistance > 1 || enemy.ammo === 0)
        ) {
          return 'move'
        } else {
      ...
      function getAttackDirection() {
        ...
        if (isEnemyAligned) {
          ...
          if (enemyOrientation === 'vertical') {
            ...
            return enemy.position[0] > playerState.position[0]
              ? 'south'
              : 'north'
          }
        }
    ...
    function getAmmo(ammoPosition) {
      ...
      var goToDirection = utils.fastGetDirection(
        playerState.position,
        ammoPosition
      )
    ...
    // Decide movement
    if (
      params.vulnerabilityLevel === 1.0 ||
      (params.vulnerabilityLevel >= 0.5 && params.canKill !== true)
    ) {
      response = defend()
