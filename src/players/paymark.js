...
var gameRunner = {
  ...
  ai: (playerState, enemiesState, gameEnvironment) => {
    ...
    if (gameEnvironment.ammoPosition.length > 0) {
      var minDist = Math.abs(
        utils.getDistance(playerState.position, gameEnvironment.ammoPosition[0])
      )
    ...
    if (hasAmmo) {
      ...
      for (i = 0; i < predictEnemys.length; i++) {
        ...
        var enDir = enemy.direction
        if (enDir == 'north') {
          predictEnemys[i].position[1]++
        }
        if (enDir == 'south') {
          predictEnemys[i].position[1]--
        }
        if (enDir == 'west') {
          predictEnemys[i].position[0]--
        }
        if (enDir == 'east') {
          predictEnemys[i].position[0]++
        }
